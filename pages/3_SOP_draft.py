...
# ── Vaade / Redigeeri tabid ───────────────────────────────────────────────────
tab_preview, tab_edit = st.tabs(["Eelvaade", "Redigeeri"])
...
with tab_preview:
    ...
    title_row = (
        f'<tr>'
        f'<td style="color:#6c757d;padding:5px 16px 8px 0;white-space:nowrap;font-size:0.85rem;vertical-align:top">Pealkiri</td>'
        f'<td style="padding:5px 0 8px 0;font-size:1.1rem;font-weight:700;color:#1e3a5f">{sop_title_text or "—"}</td>'
        f'</tr>'
    ) if sop_title_text else ""

    meta_rows = "".join(
        f'<tr>'
        f'<td style="color:#6c757d;padding:4px 16px 4px 0;white-space:nowrap;font-size:0.85rem;vertical-align:top">{k}</td>'
        f'<td style="padding:4px 0;font-weight:600;font-size:0.85rem">{v}</td>'
        f'</tr>'
    ...
    if title_row or meta_rows:
        st.markdown(
            ...
            f'<table style="border-collapse:collapse;width:100%">{title_row}{meta_rows}</table>'
            f'</div>',
            unsafe_allow_html=True,
        )

    meta_pattern = re.compile(
        r'^\*\*(?:Versioon|Kuupäev|Ettevõte|Vastutav roll):\*\*.*$', re.MULTILINE
    )
...
if automation:
    ...
    st.markdown("### AI automatiseerimisvõimalused")
...
st.markdown('<hr style="border:none;border-top:1px solid #dee2e6">', unsafe_allow_html=True)
st.markdown("### Protsessi ülevaade")
...
if diagram_raw:
    ...
    if st.button("Uuenda joonis", help="Genereerib protsessijoonise uuesti"):
        with st.spinner("Genereerin joonist..."):
            diagram_raw = generate_process_diagram(sop_draft, company)
        session["sops"][actual_idx]["process_diagram"] = diagram_raw
        save_session(session_id, session)
        st.rerun()
else:
    st.info("Protsessijoonis puudub. Genereeri SOP vestluslehel.")
...
with dl_col3:
    _diag_raw = current_sop.get("process_diagram", "")
    _diag_steps = []
    if _diag_raw:
        try:
            _m = re.search(r'\[.*\]', _diag_raw, re.DOTALL)
...
title = current_sop.get("sop_title", "")
today = date.today().strftime("%d.%m.%Y")
st.markdown('<hr style="border:none;border-top:1px solid #dee2e6">', unsafe_allow_html=True)
st.markdown("**Faili info:**")
col1, col2 = st.columns(2)
with col1:
    st.markdown(f"- **Pealkiri:** {title or '—'}")
